...
| **Cursor** | On-demand spend, included requests | 30-day / today edits, top model |
| **Grok** | SuperGrok / CLI weekly % and reset time | Plan / prepaid only if xAI sends them |

Optional **history sparks** sit under each real bar (Claude, Codex, Grok, Cursor requests). They record on every poll, including when Claude is signed out. Cursor 0/0 does not invent a spark.
...
Missing CLIs show as disabled (`grok not installed`) instead of crashing.
...
- Cursor: local auth DB + dashboard APIs (`sqlite3.exe` is bundled)
